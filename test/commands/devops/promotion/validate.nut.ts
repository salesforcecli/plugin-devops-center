...
describe('devops promotion validate NUTs', () => {
  ...
  it('displays help text', () => {
    const result = execCmd('devops promotion validate --help', { ensureExitCode: 0 });
    expect(result.shellOutput.stdout).to.include('Validate work item promotion for a pipeline stage');
  });

  it('errors when --target-stage-id is an invalid Salesforce ID format', () => {
    const result = execCmd('devops promotion validate --target-stage-id not-an-id --work-item-id 1fk000000000001AAA', {
      ensureExitCode: 1,
    });
  ...
  it('errors when --target-org is missing (valid flags supplied)', () => {
    const result = execCmd(
      'devops promotion validate --target-stage-id 1QV000000000001AAA --work-item-id 1fk000000000001AAA',
      { ensureExitCode: 1 }
    );
