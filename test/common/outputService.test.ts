...
import { OutputService } from '../../src/common/outputService';
...
describe('outputService', () => {
  test.stdout().it('prints the async deploy execution correctly', (ctx) => {
    const mockId = 'ABC';
    OutputService.printAsyncRun(mockId);
  ...
  test.stdout().it('handles the print of an undefined aor status of a deployment', (ctx) => {
    ...
    OutputService.printDeploymentStatus(aor);
  ...
  describe('deployment summary', () => {
    ...
    const outputService: OutputService = new OutputService(stubOrg.getConnection());
    ...
    describe('ad hoc deployment', () => {
      ...
      test.stdout().it('prints the correct message for a ad hoc deployment', async (ctx) => {
    ...
    describe('versioned deployment', () => {
      ...
      test.stdout().it('prints the correct message for a versioned deployment', async (ctx) => {
        ...
        const workItems2 = [workItem3, workItem4];
        sandbox.stub(ChangeBundleSelector, 'selectWorkItemsByChangeBundles').resolves([
          {
            Id: changeBundleInstall1Id,
            // eslint-disable-next-line camelcase
        ...
        expect(ctx.stdout).to.contain(
          `DevOps Center pipeline stage ${stageName} being updated with metadata associated with work items ` +
            `${workItem1Name}, ${workItem2Name} in bundle ${changeBundleInstall1VersionName}; work items ${workItem3Name}, ${workItem4Name} in bundle ${changeBundleInstall2VersionName}. ` +
