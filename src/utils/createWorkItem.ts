...
export async function createWorkItem(params: CreateWorkItemParams): Promise<CreateWorkItemResult> {
  ...
  try {
    ...
    return {
      success: true,
      workItemId: (data.workItemId ?? data.id ?? data.Id) as string | undefined,
      workItemName: (data.workItemName ?? data.name ?? data.Name) as string | undefined,
...
/**
 * Extracts a human-readable error message from a failed work item request,
 * unwrapping the various shapes the Connect API and jsforce may return.
 */
function parseErrorMessage(error: unknown): string {
  ...
  const details =
    typeof data === 'object' && data !== null && Array.isArray((data as Record<string, unknown>).body)
      ...
      : undefined;
  return details ? `${String(message)}: ${details}` : String(message);
}
